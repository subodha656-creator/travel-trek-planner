...
const router = useRouter()
  ...
  const durationInDays = selectedTrip.start_date && selectedTrip.end_date
    ? Math.ceil(
  ...
  const imageUrl = `https://${process.env.NEXT_PUBLIC_PROJECT_ID}.supabase.co/storage/v1/object/public/${'travelplanner'}/${selectedTrip.cover_image_url}`
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      if (onPhotoUpload && selectedTrip) {
        const uploadedPhotos = await onPhotoUpload(validFiles, selectedTrip.id.toString())
        ...
        setTimeout(() => {
          setPhotos(prev => prev.map(p => 
            p.isUploading ? { ...p, isUploading: false } : p
          ))
...
useEffect(() => {
  ...
  const getPhotosUploaded = async() => {
    try {
      const response = await fetch("/api/get-uploaded-photos?trip_id=" + selectedTrip.id, {
        method: "GET",
      });
  ...
  return (
    <>
    <ShareTripPopup isOpen={shareTrip} onClose={() => setShareTrip(false)} tripId={selectedTrip?.id.toString()} tripTitle={selectedTrip?.title} currentUser={currentUser} sharedUsers={sharedUsers} onSearchUsers={handleSearchUsers} />
              ...
              <div>
                ...
                >
                  ...
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">
                    {isDragOver ? 'Drop photos here!' : 'Upload Your Trip Photos'}
                  </h4>
              ...
              <div className="bg-green-50 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-700">Upload More Photos</span>
                  </Button>
                  <Button 
                  onClick={()=> router.push("/dashboard/trips/"+selectedTrip?.id)}
                    variant="outline"
                    className="w-full flex items-center gap-3 justify-start"
                  >
